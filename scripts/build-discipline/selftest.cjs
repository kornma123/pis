...
check('C1: 已清理的幽灵不再命中（6 reports + boms/cost-preview·防再引入）', () => {
  for (const p of [
...
check('C1: logs/export / users/reset-password 两个真幽灵命中', () => {
...
check('C1: 恰好 2 个幽灵（reports + boms/cost-preview 已清理·剩 2 存量·无新误报）', () => {
  assert.strictEqual(r1.violations.length, 2, `实际 ${r1.violations.length}`)
})
