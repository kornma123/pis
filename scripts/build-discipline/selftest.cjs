...
check('C1: 剩余 logs/export 真幽灵命中', () => {
  assert.ok(ghostPaths.has('GET /logs/export'), '缺 logs/export')
})
check('C1: 恰好 1 个幽灵（reports + boms/cost-preview + users/reset-password 已清理·无新误报）', () => {
  assert.strictEqual(r1.violations.length, 1, `实际 ${r1.violations.length}`)
})
