...
const backendEndpoints = R.buildBackendRegistry().endpoints
// 已清理的死调用（前端死调用移除后其对应端点不再是 C1 幽灵）——反向守卫：断言它们不再命中。
// 若哪条复现=有人又加了调无后端路由的 api，回归即红。
//   · 6 个 reports 幽灵报表端点：「清理幽灵报表端点」PR
//   · GET /boms/:/cost-preview：「清理孤儿 CostPreviewModal」PR（组件从未挂载 + 后端无此路由）
//   · POST /users/:/reset-password：PR #119 删除假端点入口，改用真实 PUT /users/:id 改密。
check('C1: 已清理的幽灵不再命中（6 reports + boms/cost-preview + users/reset-password·防再引入）', () => {
  for (const p of [
    ...
    'GET /boms/:/cost-preview',
    'POST /users/:/reset-password',
  ]) assert.ok(!ghostPaths.has(p), `${p} 应已删除、不再是幽灵（复现=前端又调了无后端路由的 api）`)
})
check('C1: logs/export 真实 consumer 与 endpoint 配对存在、不再是幽灵', () => {
  const call = calls.find((c) => c.method === 'GET' && c.relPath === '/logs/export')
  assert.ok(call, '缺前端 GET /logs/export consumer')
  const endpoint = R.matchCallToEndpoint(call, backendEndpoints)
  assert.ok(endpoint && endpoint.routeFile === 'logs-v1.1.ts', '缺后端 GET /logs/export endpoint')
  assert.ok(!ghostPaths.has('GET /logs/export'), 'logs/export 已实现，不应仍判幽灵')
})
check('C1: 真实树恰好 0 个幽灵（含 logs/export 已接通·无新误报）', () => {
  assert.strictEqual(r1.violations.length, 0, `实际 ${r1.violations.length}`)
})
...
check('B-real·6 个 reports 幽灵键 + 2 条 live-404 死线 meta 已从真实 baseline 清出（防回归）', () => {
  ...
  assert.ok(!realBaseline.keys.includes('C1|GET|/logs/export'), 'logs/export 已实现，其 C1 baseline 键应清出')
...
try {
  ...
  check('E10 exit-code·合成 C1 真幽灵 + 过期 meta → --update-baseline 照拒 exit 2（fail-closed 不破）', () => {
    // 合成前端明确调用一个 fixture 中不存在的端点；不再借真实产品端点充当幽灵，避免活实现后断言漂移。
    const missingPath = '/__bd_selftest__/missing-endpoint'
    const c1Fixture = fixture('c1-real-ghost.json', {
      calls: [{
        file: 'fixture/frontend.ts', line: 1, method: 'GET', rawPath: missingPath,
        relPath: missingPath, segs: ['__bd_selftest__', 'missing-endpoint'],
        resolvable: true, kind: 'request', fromVar: false,
      }],
      endpoints: [],
    })
    const key = `C1|GET|${missingPath}`
    const bp = fixture('deadlock-blocked.json', {
      keys: [key], targetMaxCount: 10000,
      meta: { [key]: { owner: 't', deadline: '2020-01-01' } },
    })
    assert.strictEqual(runGate(['--update-baseline'], { BD_BASELINE_PATH: bp, BD_C1_FIXTURE_PATH: c1Fixture }), 2)
  })
