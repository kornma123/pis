...
| **v1.50** | **2026-05-21** | **执行 v1.47~v1.48 补测 + v1.49 回归修复：bom-v1.1.ts 3 个 P2 根因验证全部通过（92/119 passed）；alerts-v1.1.ts 1 个 P2 根因验证通过（84/97 passed）；修复 v1.49 AppLayout.tsx 中 technician/pathologist/procurement/finance 缺少 `/alerts` 路径导致的前后端权限不一致，alerts.spec.ts 84/84 passed，bom.spec.ts 92/92 passed** |
