...
| # | 用例 ID | 优先级 | 状态 | 缺陷描述 | 涉及文件 |
|:---|:---|:---:|:---|:---|:---|
| 1 | BOM-CREATE-01/14/15/16 | P0 | 待确认 | 特定场景下 POST /boms 返回 500 | [`bom-v1.1.ts`](后端代码/server/src/routes/bom-v1.1.ts:1) |
...
| **v1.47** | `bom-v1.1.ts` | BOM-DEL-08 删除不存在返回 200→**404**<br>BLIND-BOM-10/11 XSS/SQL 注入返回 **500**<br>BLIND-BOM-16 小数用量返回 **500** | 3 | `bom.spec.ts` | ① DELETE 前检查 ID 存在性<br>② 输入清洗/参数化查询，特殊字符不再 500<br>③ 支持浮点 quantity 或明确返回 400 |
| **v1.48** | `alerts-v1.1.ts` | ALERT-HANDLE-03 / BF-ALERT-04 处理不存在返回 200→**404** | 1 | `alerts.spec.ts` | POST `/:id/handle` 前检查预警 ID 存在性，不存在返回 404 |
...
| v1.45-batch | 2026-05-20 | 执行 v1.45 batch：修复 `categories-v1.1.ts` 3 个 P2 根因（CAT-CREATE-09 重复 code→409、CAT-EDIT-06 编辑 code→400、CAT-DELETE-10 删除不存在→404），categories.spec.ts 141/141 passed，commit `84430d5` |
