...
> **待确认业务缺陷**: 0 个（2026-05-21 已全部同步，详见第六节）  
> **当前阶段**: 全部后端缺陷已修复，outbound 138/138 通过，进入“前端页面加载问题攻坚 + CI 集成”
...
| 3 | **auth/dashboard** | Sidebar 未实现角色过滤，所有角色均显示 17 个菜单 | 19 | [`AppSidebar.tsx`](前端代码/src/components/layout/AppSidebar.tsx:34) ✅ **已修复**（v1.49 `useAuth()` 按角色过滤菜单） |
...
| 14 | **auth** | 前端路由无权限守卫，无权限角色可访问受保护页面 | 5 | [`App.tsx`](前端代码/src/App.tsx:1) ✅ **已修复**（v1.49 AppLayout 路由守卫 + Login token 持久化） |

> **说明**：经逐项验证（2026-05-21），P0清单中 **14项全部已修复**（含 v1.49 前端 Sidebar 角色过滤 + AppLayout 路由守卫）。文档清单已同步。
...
| # | 模块 | 缺陷描述 | 影响用例数 |
|:---|:---|:---|:---:|
| 1 | outbound | `POST /outbound/bom` 端点未实现 | 11 | ✅ 已修复 |
...
| # | 用例 ID | 优先级 | 状态 | 缺陷描述 | 涉及文件 |
|:---|:---|:---:|:---|:---|:---|
| 1 | OUT-CREATE-PROJ-01~02 | P2 | ✅ 已通过 | `test.beforeEach` 自动补充库存，`ensureStock()` 解决库存不足 | [`outbound.spec.ts`](前端代码/e2e/outbound.spec.ts:188) |
| 2 | OUT-CREATE-PROJ-10 | P2 | ✅ 已通过 | `ensureStock()` 预置库存，并发场景不再 422 | 同上 |
| 3 | OUT-CREATE-PROJ-17~18 | P0 | ✅ 已修复 | quantity=0/负数已校验返回 400 | [`outbound-v1.1.ts`](后端代码/server/src/routes/outbound-v1.1.ts:57) |
| 4 | OUT-CREATE-PROJ-19 | P2 | ✅ 已通过 | `ensureStock()` 预置库存，成本归集可正常验证 | 同上 |
| 5 | OUT-CREATE-TRF-06 | P2 | ✅ 已通过 | `ensureStock()` 预置库存，并发调拨不再 422 | 同上 |
...
| 8 | OUT-CREATE-SCRAP-06 | P2 | ✅ 已通过 | `ensureStock()` 预置库存，并发报废不再 422 | 同上 |
| 9 | OUT-CREATE-SCRAP-08 | P0 | ✅ 已修复 | 负数报废已校验返回 400 | 同上 |
| 10 | OUT-BOM-01~11 | P3 | ✅ 已修复 | `POST /outbound/bom` 端点已实现（FIFO批次分配、事务保护、库存扣减） | [`outbound-v1.1.ts`](后端代码/server/src/routes/outbound-v1.1.ts:165) |
| 11 | OUT-PAGE-03 | P1 | ✅ 已修复 | page=0 已修正为 1 | [`outbound-v1.1.ts`](后端代码/server/src/routes/outbound-v1.1.ts:16) |
| 12 | BF-OUT-08 | P3 | ✅ 已修复 | `/outbound/bom` 正常返回 201/400/422 | 同上 |
| 13 | BF-OUT-13 | P3 | ✅ 已修复 | BOM 出库后成本归集可验证 | 同上 |
...
| **v1.51** | **2026-05-21** | **实现 BOM 一键出库：POST /outbound/bom 端点（FIFO 批次分配、事务保护、库存扣减、成本归集）；修复 outbound 响应缺少 `createdAt` 字段；修复 outboundNo 正则匹配实际格式；数据库补充 BOM items 使 E2E 测试通过；outbound.spec.ts 138/138 全部通过** |
