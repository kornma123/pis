...
describe('materialApi 响应真值（Issue71）', () => {
  ...
  it.each([
    ...
    ['price 为 NaN', materialRow({ price: NaN })],
  ...
  it('getNextCode：fail-closed（未解包的错误 envelope 不冒充成功）', async () => {
...
describe('bomApi 响应真值（Issue71）', () => {
  it('合法 0 / null / 空列表保真：materialCount 0、unitCost null、supportableSamples null', async () => {
    get.mockResolvedValue(
      pagePayload([bomRow({ materialCount: 0, unitCost: null, supportableSamples: null })], 1) as never
    )
  ...
  it('endpoint-specific：BOM 行缺 materialCount 必须 fail-closed（不得复用其它 parser 放行）', async () => {
    get.mockResolvedValue(pagePayload([bomRow({ materialCount: undefined })]) as never)
    await expect(bomApi.getList({})).rejects.toThrow()
  })

  it.each([
    ['unitCost 为负数', bomRow({ unitCost: -0.1 })],
    ['supportableSamples 非整数', bomRow({ supportableSamples: 1.5 })],
  ...
  it('detail：合法空 materials/versionHistory 与 usagePerSample=0、price=null 保真', async () => {
    get.mockResolvedValue({
      id: 'bom-1',
      code: 'BOM-001',
      name: 'HE基础套',
    ...
    expect(res.materials[0].price).toBeNull()
    expect(res.versionHistory).toEqual([])
  })

  it('detail：fail-closed（materials 非数组 / usagePerSample 负数 / versionHistory 缺失）', async () => {
    get.mockResolvedValue({
      id: 'bom-1',
      code: 'BOM-001',
      name: 'HE基础套',
    ...
    get.mockResolvedValue({
      ...
      type: 'he',
      status: 'active',
      materials: [{ id: 'mat-1', usagePerSample: -1, stock: 0, costRatio: 0 }],
      versionHistory: [],
    } as never)
    await expect(bomApi.getDetail('bom-1')).rejects.toThrow()
