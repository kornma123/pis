...
export const materialApi = {
  getList: (params?: PageParams & { categoryId?: string; supplierId?: string; status?: string }) =>
    request.get('/materials', { params }).then(parseMaterialListResponse),
  getDetail: (id: string) => request.get(`/materials/${id}`).then(parseMaterialDetailResponse),
  getNextCode: (categoryId: string) =>
    request.get('/materials/next-code', { params: { categoryId } }).then(parseNextCodeResponse),
...
export const projectApi = {
  getList: (params?: PageParams & { type?: string; status?: string; bomFilter?: string }) =>
    request.get('/projects', { params }).then(parseProjectListResponse),
  getDetail: (id: string) => request.get(`/projects/${id}`).then(parseProjectDetailResponse),
...
export const bomApi = {
  getList: (params?: PageParams & { type?: string; status?: string }) =>
    request.get('/boms', { params }).then(parseBomListResponse),
  getDetail: (id: string) => request.get(`/boms/${id}`).then(parseBomDetailResponse),
