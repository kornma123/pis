...
export function StocktakingAdjustModal({ open, row, reason, note, submitting, onChangeReason, onChangeNote, onSubmit, onClose }: Props) {
      ...
      <div className="bg-white rounded-xl shadow-xl w-full max-w-2xl mx-4 overflow-hidden flex flex-col max-h-[90vh]">
        ...
        <div className="p-6 overflow-y-auto space-y-4">
          <div className="bg-amber-50 border border-amber-100 rounded-lg p-4 flex items-start gap-3">
            ...
            <div>
              <div className="text-sm font-medium text-amber-900">这笔盘点还没入账</div>
              <div className="text-xs text-amber-700 mt-0.5">选好差异原因后确认，系统才会把库存改成实盘数，并记录这次调整</div>
            </div>
          </div>
          <div>
            ...
            <div className="border border-gray-200 rounded-lg divide-y divide-gray-100">
              <div className="p-4 flex items-center gap-4">
                <div className={`w-8 h-8 rounded-full flex items-center justify-center shrink-0 ${row.difference > 0 ? 'bg-green-100 text-green-600' : 'bg-red-100 text-red-600'}`}>
                  {row.difference > 0 ? '+' : '-'}
                </div>
                ...
                <select
                  ...
                  className="px-3 py-1.5 border border-gray-300 rounded-md text-sm bg-white focus:outline-none focus:ring-2 focus:ring-blue-500 w-32 shrink-0"
                >
          ...
          <div className="bg-gray-50 rounded-lg p-4">
            <div className="flex items-center justify-between">
              <span className="text-xs text-gray-500">调整后库存</span>
              <span className="text-sm font-semibold">账面 {row.systemStock} → <span className="text-blue-600">实盘 {row.actualStock}</span></span>
            </div>
          </div>
          <div>
        ...
        <div className="flex items-center justify-end gap-3 px-6 py-4 border-t border-gray-200 shrink-0">
          ...
          <button
            ...
            className="px-4 py-2 bg-blue-500 text-white text-sm rounded-md hover:bg-blue-600 flex items-center gap-1 disabled:opacity-50 disabled:cursor-not-allowed"
          >
            <CheckCircle className="w-4 h-4" />{submitting ? '处理中…' : '确认调整'}
          </button>
        </div>
      </div>
    </div>
