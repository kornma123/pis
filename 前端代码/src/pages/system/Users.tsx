  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex items-start justify-between">
        ...
        <button onClick={openCreate} className="inline-flex items-center gap-2 px-4 py-2.5 bg-blue-500 text-white rounded-md hover:bg-blue-600 text-sm font-medium shadow-sm transition-all">
      ...
      <div className="grid grid-cols-4 gap-4">
        <div className="bg-white p-5 rounded-lg shadow-sm border border-gray-200 transition-all hover:shadow-md hover:-translate-y-0.5">
          <div className="text-[28px] font-semibold text-gray-900 leading-tight tracking-tight">{stats.totalUsers}</div>
          <div className="text-[13px] text-gray-500 mt-1">用户总数</div>
        </div>
        <div className="bg-white p-5 rounded-lg shadow-sm border border-gray-200 transition-all hover:shadow-md hover:-translate-y-0.5">
          <div className="text-[28px] font-semibold text-[#22c55e] leading-tight tracking-tight">{stats.activeUsers}</div>
          <div className="text-[13px] text-gray-500 mt-1">启用用户</div>
        </div>
        <div className="bg-white p-5 rounded-lg shadow-sm border border-gray-200 transition-all hover:shadow-md hover:-translate-y-0.5">
          <div className="text-[28px] font-semibold text-gray-500 leading-tight tracking-tight">{stats.inactiveUsers}</div>
          <div className="text-[13px] text-gray-500 mt-1">停用用户</div>
        </div>
        <div className="bg-white p-5 rounded-lg shadow-sm border border-gray-200 transition-all hover:shadow-md hover:-translate-y-0.5">
          <div className="text-[28px] font-semibold text-[#3b82f6] leading-tight tracking-tight">{stats.adminUsers}</div>
          <div className="text-[13px] text-gray-500 mt-1">管理员</div>
        </div>
      </div>
          ...
          <div className="p-4 flex flex-col gap-3">
            {roles.map(role => (
              <div
                ...
                className={`p-4 rounded-lg border cursor-pointer transition-all ${selectedRoleId === role.id ? 'border-[#3b82f6] bg-[#eff6ff]' : 'border-gray-200 bg-white hover:border-gray-300'}`}
              >
                ...
                <div className="flex flex-wrap gap-1">
                  {role.permissions.slice(0, 3).map(p => (
                    <span key={p} className="text-[11px] px-2 py-0.5 rounded-full bg-[#f3f4f6] text-gray-700">{p}</span>
                  ))}
                  {role.permissions.length > 3 && (
                    <span className="text-[11px] px-2 py-0.5 rounded-full bg-[#f3f4f6] text-gray-700">+{role.permissions.length - 3}</span>
                  )}
                </div>
              </div>
            ))}
              ...
              <thead>
                <tr className="bg-gray-50 border-b border-gray-200">
                  <th className="px-4 py-3 text-left">
                    <input type="checkbox" className="rounded border-gray-300 text-[#3b82f6] focus:ring-blue-500" />
                  </th>
              ...
              <tbody className="divide-y divide-[#e5e7eb]">
                {loading ? (
                  ...
                  <tr key={row.id} className="hover:bg-gray-50 transition-colors">
                    ...
                    <td className="px-4 py-3.5">
                      <span className={`inline-flex items-center px-3 py-1 rounded-full text-xs font-medium ${row.status === 'active' ? 'bg-[#f0fdf4] text-[#22c55e]' : 'bg-[#f3f4f6] text-gray-500'}`}>
                    ...
                    <td className="px-4 py-3.5">
                      <div className="flex items-center gap-1">
                        ...
                        <button onClick={() => openEdit(row)} className="h-8 px-3 text-[13px] text-gray-700 hover:bg-gray-100 rounded-md transition-colors">编辑</button>
                        <button onClick={() => handleToggleStatus(row)} className="h-8 px-3 text-[13px] text-gray-700 hover:bg-gray-100 rounded-md transition-colors">
                          {row.status === 'active' ? '停用' : '启用'}
                        </button>
                        <button onClick={() => handleResetPassword(row.id)} className="h-8 px-3 text-[13px] text-gray-700 hover:bg-gray-100 rounded-md transition-colors">重置密码</button>
                        <button onClick={() => handleDelete(row.id)} className="h-8 px-3 text-[13px] text-red-500 hover:bg-[#fef2f2] rounded-md transition-colors">删除</button>
                      </div>
                    </td>
                  </tr>
                ))}
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-[rgba(17,24,39,0.6)]">
          <div className="bg-white rounded-xl shadow-xl w-full max-w-lg mx-4 max-h-[90vh] overflow-hidden flex flex-col">
            <div className="flex items-center justify-between px-6 py-5 border-b border-gray-200">
              <h3 className="text-lg font-semibold text-gray-900">{modalType === 'create' ? '新建用户' : '编辑用户'}</h3>
              <button onClick={() => setModalType(null)} className="w-9 h-9 flex items-center justify-center text-gray-500 hover:bg-gray-100 hover:text-gray-900 rounded-md transition-all">
                <X className="w-5 h-5" />
              </button>
            </div>
            <div className="p-6 overflow-y-auto">
              <div className="flex gap-5 mb-5">
                <div className="flex-1">
                  <label className="block text-[13px] font-medium text-gray-700 mb-1.5">用户名 {modalType === 'create' && <span className="text-red-500">*</span>}</label>
                  <input
                    value={form.username}
                    onChange={e => setForm({ ...form, username: e.target.value })}
                    readOnly={modalType === 'edit'}
                    className={`w-full h-10 px-3 text-sm text-gray-900 border border-gray-300 rounded-md outline-none transition-all focus:border-blue-500 focus:ring-[3px] focus:ring-blue-500/10 ${modalType === 'edit' ? 'bg-gray-50 text-[#9ca3af]' : 'bg-white'}`}
                  />
                </div>
                <div className="flex-1">
            ...
            <div className="flex items-center justify-end gap-3 px-6 py-4 border-t border-gray-200 bg-gray-50">
              <button onClick={() => setModalType(null)} className="h-10 px-4 text-sm text-gray-700 bg-white border border-gray-300 rounded-md hover:bg-gray-50 shadow-sm transition-all">取消</button>
              {modalType === 'edit' && (
                <button onClick={() => editingId && handleResetPassword(editingId)} className="h-10 px-4 text-sm text-gray-700 bg-white border border-gray-300 rounded-md hover:bg-gray-50 shadow-sm transition-all">重置密码</button>
              )}
              <button onClick={handleSubmit} className="h-10 px-4 text-sm font-medium text-white bg-blue-500 rounded-md hover:bg-blue-600 shadow-sm transition-all">
          ...
          <div className="bg-white rounded-xl shadow-xl w-full max-w-2xl mx-4 max-h-[90vh] overflow-hidden flex flex-col">
            ...
            <div className="p-6 overflow-y-auto">
              <div className="flex items-center gap-4 mb-6">
                ...
                <div className="flex-1">
                  <div className="text-lg font-semibold text-gray-900">{detailUser.realName}</div>
                  <div className="text-[13px] text-gray-500">用户名: {detailUser.username}</div>
                </div>
                <span className={`inline-flex items-center px-3 py-1 rounded-full text-xs font-medium ${detailUser.status === 'active' ? 'bg-[#f0fdf4] text-[#22c55e]' : 'bg-[#f3f4f6] text-gray-500'}`}>
              ...
              <div className="grid grid-cols-3 gap-5">
                <div>
                  ...
                  <div className="text-[15px] font-semibold text-gray-900">{detailUser.role}</div>
                </div>
                <div>
                  <div className="text-xs text-gray-500 mb-1">部门</div>
                  <div className="text-[15px] font-semibold text-gray-900">{detailUser.department || '-'}</div>
                </div>
                <div>
                  <div className="text-xs text-gray-500 mb-1">联系电话</div>
                  <div className="text-[15px] font-semibold text-gray-900">{detailUser.phone || '-'}</div>
                </div>
                <div>
                  <div className="text-xs text-gray-500 mb-1">电子邮箱</div>
                  <div className="text-[15px] font-semibold text-gray-900">{detailUser.email || '-'}</div>
                </div>
                <div>
                  <div className="text-xs text-gray-500 mb-1">创建时间</div>
                  <div className="text-[15px] font-semibold text-gray-900">{detailUser.createdAt ? new Date(detailUser.createdAt).toLocaleString() : '-'}</div>
                </div>
                <div>
                  <div className="text-xs text-gray-500 mb-1">最后登录</div>
                ...
                {detailUser.permissions && detailUser.permissions.length > 0 ? (
                  <div className="border border-gray-200 rounded-lg overflow-hidden">
                    <div className="flex items-center justify-between px-4 py-3 bg-gray-50 border-b border-gray-200">
                      ...
                      <span className="text-xs text-gray-500">{detailUser.permissions.length} 项权限</span>
                    </div>
                    <div className="p-4">
            ...
            <div className="flex items-center justify-end gap-3 px-6 py-4 border-t border-gray-200 bg-gray-50">
              <button onClick={() => setModalType(null)} className="h-10 px-4 text-sm text-gray-700 bg-white border border-gray-300 rounded-md hover:bg-gray-50 shadow-sm transition-all">关闭</button>
              <button onClick={() => { setModalType(null); openEdit(detailUser); }} className="h-10 px-4 text-sm font-medium text-white bg-blue-500 rounded-md hover:bg-blue-600 shadow-sm transition-all">编辑</button>
            </div>
          </div>
        </div>
      )}
