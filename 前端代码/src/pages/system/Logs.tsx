...
const LOG_TYPES = [
  ...
  { value: 'logout', label: '登出', className: 'bg-[#f3f4f6] text-[#6b7280]' },
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex items-start justify-between">
        <div>
          <h1 className="text-[28px] font-semibold text-[#111827] tracking-tight leading-tight">操作日志</h1>
          <p className="text-sm text-[#6b7280] mt-1">查看系统操作记录，追踪用户行为</p>
        </div>
        <button onClick={() => setShowExport(true)} className="inline-flex items-center gap-2 px-4 py-2.5 bg-white text-[#374151] border border-[#d1d5db] rounded-md hover:bg-[#f9fafb] text-sm font-medium shadow-sm transition-all">
      ...
      <div className="bg-white rounded-lg shadow-sm border border-[#e5e7eb] overflow-hidden">
        <div className="px-5 py-4 border-b border-[#e5e7eb] bg-[#f9fafb] flex items-center justify-between flex-wrap gap-3">
          <span className="text-base font-semibold text-[#111827]">操作记录</span>
          <div className="flex items-center gap-3 flex-wrap">
            ...
            <input
              ...
              className="h-10 px-3 text-sm text-[#111827] bg-white border border-[#e5e7eb] rounded-md outline-none transition-all focus:border-[#3b82f6] focus:shadow-[0_0_0_3px_rgba(59,130,246,0.1)]"
            />
            <span className="text-[#6b7280]">至</span>
            <input
              type="date"
              value={endDate}
              ...
              {loading ? (
                ...
                return (
                  <tr key={row.id} className="hover:bg-[#f9fafb] transition-colors">
                    <td className="px-4 py-3.5 font-mono text-[13px] text-[#374151]">{new Date(row.createdAt).toLocaleString()}</td>
                    <td className="px-4 py-3.5">
                      <div className="flex items-center gap-2">
                        ...
                        <div>
                          <div className="text-sm font-medium text-[#111827]">{row.username}</div>
                        </div>
                      </div>
                    </td>
                    <td className="px-4 py-3.5">
                    ...
                    <td className="px-4 py-3.5">
                      <span className="text-xs px-2.5 py-1 rounded-full bg-[#f3f4f6] text-[#374151] font-medium">{getModuleLabel(row.requestData?.module as string || '')}</span>
                    </td>
                    <td className="px-4 py-3.5">
                      <div className="text-sm text-[#111827]">{row.description}</div>
                      {row.requestData && (
                        <div className="text-xs text-[#6b7280] mt-0.5">{JSON.stringify(row.requestData).slice(0, 60)}...</div>
                      )}
                    </td>
                    <td className="px-4 py-3.5 font-mono text-[13px] text-[#6b7280]">{row.ip}</td>
                    <td className="px-4 py-3.5">
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-[rgba(17,24,39,0.6)]">
          <div className="bg-white rounded-xl shadow-xl w-full max-w-2xl mx-4 max-h-[90vh] overflow-hidden flex flex-col">
            <div className="flex items-center justify-between px-6 py-5 border-b border-[#e5e7eb]">
              <h3 className="text-lg font-semibold text-[#111827]">操作详情</h3>
              <button onClick={() => setShowDetail(false)} className="w-9 h-9 flex items-center justify-center text-[#6b7280] hover:bg-[#f3f4f6] hover:text-[#111827] rounded-md transition-all">
                <X className="w-5 h-5" />
              </button>
            </div>
            <div className="p-6 overflow-y-auto">
              <div className="grid grid-cols-2 gap-5 mb-6">
                <div>
                  <div className="text-xs text-[#6b7280] mb-1">操作时间</div>
                  <div className="text-[15px] font-semibold text-[#111827] font-mono">{new Date(detailLog.createdAt).toLocaleString()}</div>
                </div>
                <div>
                  <div className="text-xs text-[#6b7280] mb-1">操作类型</div>
                ...
                <div>
                  <div className="text-xs text-[#6b7280] mb-1">操作用户</div>
                  <div className="text-[15px] font-semibold text-[#111827]">{detailLog.username}</div>
                </div>
                <div>
                  <div className="text-xs text-[#6b7280] mb-1">操作模块</div>
                  <div className="text-[15px] font-semibold text-[#111827]">{getModuleLabel(detailLog.requestData?.module as string || '')}</div>
                </div>
                <div>
                  <div className="text-xs text-[#6b7280] mb-1">IP地址</div>
                  <div className="text-[15px] font-semibold text-[#111827] font-mono">{detailLog.ip}</div>
                </div>
                <div>
                  <div className="text-xs text-[#6b7280] mb-1">浏览器</div>
                  <div className="text-[15px] font-semibold text-[#111827]">{detailLog.userAgent || '-'}</div>
                </div>
              </div>

              <div className="mb-6">
                <h4 className="text-sm font-semibold text-[#111827] mb-3">操作内容</h4>
                <div className="bg-[#f9fafb] rounded-lg p-4">
                  <div className="text-sm text-[#111827] mb-1">{detailLog.description}</div>
                  {detailLog.requestData && (
                    <div className="text-[13px] text-[#6b7280]">{JSON.stringify(detailLog.requestData)}</div>
                  )}
                </div>
              </div>
              ...
              {detailLog.requestData && (
                <div>
                  <h4 className="text-sm font-semibold text-[#111827] mb-3">变更详情</h4>
                  <div className="border border-[#e5e7eb] rounded-lg overflow-hidden">
                    <div className="bg-[#f9fafb] px-4 py-2 text-xs text-[#6b7280] border-b border-[#e5e7eb]">请求数据</div>
                    <table className="w-full text-sm">
                      <tbody className="divide-y divide-[#e5e7eb]">
                        {Object.entries(detailLog.requestData).map(([key, value]) => (
                          <tr key={key}>
                            <td className="px-4 py-2.5 w-[140px] bg-[#f9fafb] font-medium text-[#374151]">{key}</td>
                            <td className="px-4 py-2.5 text-[#111827]">{String(value)}</td>
                          </tr>
                        ))}
                      </tbody>
                    </table>
            ...
            <div className="flex items-center justify-end gap-3 px-6 py-4 border-t border-[#e5e7eb] bg-[#f9fafb]">
              <button onClick={() => setShowDetail(false)} className="h-10 px-4 text-sm text-[#374151] bg-white border border-[#d1d5db] rounded-md hover:bg-[#f9fafb] shadow-sm transition-all">关闭</button>
            </div>
          </div>
        </div>
      )}
      ...
      {showExport && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-[rgba(17,24,39,0.6)]">
          <div className="bg-white rounded-xl shadow-xl w-full max-w-lg mx-4 max-h-[90vh] overflow-hidden flex flex-col">
            <div className="flex items-center justify-between px-6 py-5 border-b border-[#e5e7eb]">
              <h3 className="text-lg font-semibold text-[#111827]">导出日志</h3>
              <button onClick={() => setShowExport(false)} className="w-9 h-9 flex items-center justify-center text-[#6b7280] hover:bg-[#f3f4f6] hover:text-[#111827] rounded-md transition-all">
                <X className="w-5 h-5" />
              </button>
            </div>
            <div className="p-6 overflow-y-auto">
              <div className="mb-5">
                <label className="block text-[13px] font-medium text-[#374151] mb-1.5">导出时间范围</label>
                <div className="flex items-center gap-3">
                  <input
                    type="date"
                    value={exportForm.startDate}
                    onChange={e => setExportForm({ ...exportForm, startDate: e.target.value })}
                    className="flex-1 h-10 px-3 text-sm text-[#111827] bg-white border border-[#d1d5db] rounded-md outline-none transition-all focus:border-[#3b82f6] focus:shadow-[0_0_0_3px_rgba(59,130,246,0.1)]"
                  />
                  <span className="text-[#6b7280]">至</span>
                  <input
                    type="date"
              ...
              <div className="mb-5">
                <label className="block text-[13px] font-medium text-[#374151] mb-2">导出格式</label>
                <div className="flex gap-3">
                  <label
                    onClick={() => setExportForm({ ...exportForm, format: 'xlsx' })}
                    className={`flex-1 flex items-center gap-2 p-3 border-2 rounded-lg cursor-pointer transition-all ${exportForm.format === 'xlsx' ? 'border-[#3b82f6] bg-[#eff6ff]' : 'border-[#e5e7eb]'}`}
                  >
                  ...
                  <label
                    ...
                    className={`flex-1 flex items-center gap-2 p-3 border-2 rounded-lg cursor-pointer transition-all ${exportForm.format === 'csv' ? 'border-[#3b82f6] bg-[#eff6ff]' : 'border-[#e5e7eb]'}`}
                  >
                    <input type="radio" checked={exportForm.format === 'csv'} readOnly className="text-[#3b82f6]" />
                    <span className="text-sm text-[#111827]">CSV (.csv)</span>
                  </label>
                </div>
              </div>

              <div>
                <label className="block text-[13px] font-medium text-[#374151] mb-2">导出内容</label>
                <div className="flex flex-col gap-2">
                  <label className="flex items-center gap-2 text-sm text-[#111827] cursor-pointer">
                    <input type="checkbox" checked={exportForm.includeBasic} onChange={e => setExportForm({ ...exportForm, includeBasic: e.target.checked })} className="rounded border-[#d1d5db] text-[#3b82f6] focus:ring-[#3b82f6] w-4 h-4" />
                    基本信息（时间、用户、类型、模块）
                  </label>
                  <label className="flex items-center gap-2 text-sm text-[#111827] cursor-pointer">
                    <input type="checkbox" checked={exportForm.includeDetail} onChange={e => setExportForm({ ...exportForm, includeDetail: e.target.checked })} className="rounded border-[#d1d5db] text-[#3b82f6] focus:ring-[#3b82f6] w-4 h-4" />
                    操作详情
                  </label>
                  <label className="flex items-center gap-2 text-sm text-[#111827] cursor-pointer">
                    <input type="checkbox" checked={exportForm.includeIP} onChange={e => setExportForm({ ...exportForm, includeIP: e.target.checked })} className="rounded border-[#d1d5db] text-[#3b82f6] focus:ring-[#3b82f6] w-4 h-4" />
                    IP地址和设备信息
                  </label>
                  <label className="flex items-center gap-2 text-sm text-[#111827] cursor-pointer">
                    <input type="checkbox" checked={exportForm.includeDiff} onChange={e => setExportForm({ ...exportForm, includeDiff: e.target.checked })} className="rounded border-[#d1d5db] text-[#3b82f6] focus:ring-[#3b82f6] w-4 h-4" />
                    变更前后数据对比
                  </label>
                </div>
              </div>
            ...
            <div className="flex items-center justify-end gap-3 px-6 py-4 border-t border-[#e5e7eb] bg-[#f9fafb]">
              <button onClick={() => setShowExport(false)} className="h-10 px-4 text-sm text-[#374151] bg-white border border-[#d1d5db] rounded-md hover:bg-[#f9fafb] shadow-sm transition-all">取消</button>
              <button onClick={handleExport} className="h-10 px-4 text-sm font-medium text-white bg-[#3b82f6] rounded-md hover:bg-[#2563eb] shadow-sm transition-all">导出</button>
            </div>
          </div>
        </div>
      )}
