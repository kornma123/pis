...
export function useBOMPage() {
  ...
  const openDetail = async (row: BOM) => {
    try {
      ...
      /* Issue71 fail-closed：详情失败时不把列表行冒充详情、不打开详情弹窗 */
    }
  }
  ...
  const handleCopy = async () => {
    ...
    try {
      ...
      const payload = {
        ...
        status: 'active',
        materialCount: copyForm.copyMaterials ? source.materials.length : 0,
      ...
      await bomApi.create(payload as unknown as Partial<BOM>)
