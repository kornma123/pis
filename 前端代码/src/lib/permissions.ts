...
export function getAccessiblePaths(): string[] {
  ...
  if (caps) {
    ...
    // LIS 病例导入：口径/工作量数据源，管理员+财务（后端 requireAnyRole('admin','finance') 守卫，见 lis-cases 路由）
    if (roles.includes('admin') || roles.includes('finance')) paths.push('/lis-import')
    return paths
  }
...
export const ROLE_MENU_MAP: Record<string, string[]> = {
  admin: [
    ...
    '/purchase-orders', '/suppliers', '/locations', '/users', '/roles', '/logs', '/partner-config', '/lis-import', '/import-console', '/import-wizard',
    // ABC 成本核算（移植）
  ...
  finance: [
    '/', '/inventory', '/supplier-returns', '/reconciliation', '/cost-analysis', '/categories', '/alerts', '/partner-config', '/lis-import', '/import-console', '/import-wizard',
    // ABC 成本核算（移植）
    '/abc/dashboard', '/abc/slide-cost', '/abc/profitability', '/abc/activity-centers', '/equipment', '/labor-times', '/indirect-costs',
  ],
  pathologist: [
