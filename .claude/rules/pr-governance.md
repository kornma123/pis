...
> **优先级**：P0 — 强制。每个会话启动读 `session-log.md` 时，连带用 `gh pr list` 核对本文件「活跃 PR 看板」（**对账认知真实状态即可，差异随下一个实质 PR 捎带、不即改看板、不为此单开 PR**——见 §1 第 8 条）。
...
5. **合并顺序写下来**：见下方看板「合并顺序」。合并任意 PR 前，先核对它是不是当前可合的最上游项；合完**只处理下游 PR 的 base 重定向**（看板状态**不实时回改**，见第 8 条）。
...
## 4. 活跃 PR 看板（关系/顺序/风险的人读视图，非实时事实源）

> **实时 open/merged 真相以 `gh pr list` 为准**；本表是「关系 + 顺序 + 风险」的人读视图，**状态列尽力标注、不追求实时精确、合并后不回改**（差异随下一个实质 PR 捎带更正，**禁止为纯看板回填单开 PR**——见 §1 第 8 条）。行内已带各自状态日期 + git log 可查，**不再单独维护标头时间戳**（原标头"更新时间：2026-06-29"与表内 2026-07-01 行自相矛盾，已删——防漂移文件自己别漂）。
...
| 合并序 | PR | 分支 → base | 状态 | 关系 / 风险 | 标签 |
|---|---|---|---|---|---|
| — | [#65](https://github.com/Mazikorn/Coreone-Procurement-Sales-and-Inventory-PSI-Management-System/pull/65) | `claude/magical-sutherland-e69f26` → `master` | ✅ **MERGED**(2026-07-03, merge commit `f750b280`) | **独立**（非栈式，无上下游；off master `877b3932`）。实现项 **I-1**（PM 已拍「配置类保留」）。**纯前端·零后端·golden 天然零回归**。8 个 ABC 孤儿页接回侧栏（6 配置页+成本异常台账+成本审计追溯）+ `NAV_PATH_MODULE`/`ROLE_MENU_MAP` 权限映射。仅改 2 前端文件（`permissions.ts`/`AppSidebar.tsx`）。可达性⟺后端授权对齐：配置类映 `abc_config`（同既有 activity-centers）、alerts/audit 映 `abc_dashboard`；**共旅不变量**（持 abc_config 者必同持 abc_dashboard:R）经 SEED_MATRIX+运行库+逐 6 种子用户×8 路径 live HTTP 验证=**0 破链**。前端 tsc/build 绿·permissions vitest(15+7)绿·整仓零新增失败；后端 vitest 89/757 绿·golden ¥13,152+¥27,870 零回归；真跑端到端 8 页可点开+落库+审计留痕。独立复核=codex 异构(high·拆多请求)+Workflow 3-lens 对抗面板+inline live harness 三引擎一致；采纳 2 项 LOW 命名消歧。合并前 merge origin/master 消治理文档 append 冲突（保留 #62+#65 双看板段）；vitest required 绿(1m9s)后 `--merge --admin`（e2e 非 required·pending 不阻断）落 master。 | merge-order/1 |
...
*与 `CLAUDE.md`、`coreone-guardrails.md`、`session-log.md` 配套。看板是「关系/顺序/风险」的人读视图、**非实时事实源**——实时 open/merged 真相以 `gh pr list` 为准；PR body/标签是关系信息在 GitHub 的镜像。*
