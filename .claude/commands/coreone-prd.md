---
description: 用 Claude Code 把一个 GitHub Issue 整理成受控 COREONE PRD 草稿
argument-hint: "[#Issue 或完整 URL]"
---

按 `docs/Claude-Code-PRD-GitHub协作范式.md` 处理 `$ARGUMENTS`。
...
3. 按 `docs/github-issue-pr-management-loop.md` 的 `coreone-owner` 受控块认领：Issue 初始保持 `unassigned`，若要修改仓库则用 `claude-task.cjs start --stage=prd --claim=true ...` 在 preflight 通过后原子更新 body 与认领评论。没有 body 写权限时保持“未认领”。
4. 只认领 PRD 文档和本任务明确允许的治理文件。
