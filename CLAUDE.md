...
**第 0 步·先同步（否则会拿着过时规则/进度干活）**：会话启动先 `git fetch origin`，确认当前工作区**不落后 `origin/master`**（`git log --oneline HEAD..origin/master` 应为空）。若落后——尤其在**主仓或复用的旧 worktree** 里启动——要么先同步到 origin/master，要么直接读 `origin/master` 版规则文件（`git show origin/master:<路径>`）。新工作本就从 `origin/master` 开 worktree，天然新鲜；**审查/复用旧树的会话最易踩过时**（本规则由 2026-07-06 机制审查补：主仓曾落后 origin/master 32 提交、揣着 §1.8 前的旧规则开工）。

**然后读取 `.claude/session-log.md`**，了解当前工作进度、已完成项和待办事项，避免重复工作或遗漏上下文（**读法见该文件头部规则：读头部规则块 + 末尾若干段 + 承重指针，不必通读全文**）。
...
> **权威在工作模型四段**（讨论循环 + 逼 AI 摊假设 + 真数据产手核答案 → BDD/TDD → mockup 真人 → 独立复核）。下面是它在本项目的落地速记；**别调用 planner/code-reviewer 等不存在的"专业代理"**（见上文「可用代理」订正）。

1. **讨论 + 摊假设** — 动手前把假设/猜测/可能误解摊开问 PM，在真数据上跑出结果给 PM 确认；讨论是循环，不是一问一答。
2. **TDD 守黄金** — 碰钱/口径先按手核答案写会红的 vitest 断言，再实现，再重构（黄金 ¥13,152 / ¥27,870 不回归）。
3. **独立复核** — 代码/结论用不共享盲点的第二视角再核：codex 异构轴（`.claude/rules/codex-cli-usage.md`）或 Claude 侧 Workflow 对抗面板；已装技能可用 `/code-review`、`/security-review`。
4. **建完给真人** — 前端走 mockup 先行红线（未定稿不写真代码），真页面摆到真实用户面前走查。
5. **E2E** — 用户流程变更后更新/运行 E2E 测试（当前回归网状态见 `.claude/rules/coreone-guardrails.md`「E2E 现状」）。
