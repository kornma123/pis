...
- 项目 hooks 要求 Claude Code `>= 2.1.139`；另一台设备先用 `claude --version` 核对，低于该版本不得把未触发的 hook 当作流程已生效。
- 由 Codex 或 PM 启动、续接或监督的本地 CLI 会话，必须同时遵守 [Claude Code CLI 监督协议](docs/claude-code-cli-supervision.md)：只接受同一桌面终端写入与应用回读已经证明的前台会话；工具侧 PTY 不得冒充用户可见终端。
