...
| 规则 | 说明 |
|------|------|
| 无关联校验 | 即使该供应商有物料关联或有采购订单，仍可删除 |
| 悬空引用 | 删除后，`materials.supplier_id` 成为悬空引用，物料详情中供应商名称可能显示为空 |
| 不级联删除 | 不删除关联的物料、采购订单、入库记录 |
...
| 删除后物料仍关联 | materials.supplier_id 成为悬空引用 |
