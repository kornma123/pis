...
| 步骤 | 操作 | 说明 |
|------|------|------|
| 1 | `BEGIN IMMEDIATE` 后查询未删除角色 | 404 若不存在 |
| 2 | 保护系统管理员角色 | `code = 'admin'` 返回 403 |
| 3 | 锁内查询有效分配 | 启用且未删除用户经 `users.role`、`users.primary_role` 或 `user_roles.role_code` 持有该角色 |
| 4 | 有效分配存在 | 409 `ENTITY_IN_USE`，回滚且零业务写 |
| 5 | 无有效分配 | UPDATE `is_deleted = 1` 后 COMMIT，返回 200 |

#### 5.4.2 隐含规则显式化

| 规则 | 说明 |
|------|------|
| 有效分配阻断 | 启用且未删除用户仍持有角色时返回 409 `ENTITY_IN_USE` |
| 停用/历史分配放行 | `status = 0` 的停用用户或 `is_deleted = 1` 的历史用户不阻断；用户行与 `user_roles` 行不级联、不改写 |
| 多角色兼容 | 三条持有路径任一命中即阻断，避免只查 `users.role` 漏掉 `primary_role`/`user_roles` |
| 原子性 | 查询角色、锁内查有效分配、软删除写入处于同一个 `BEGIN IMMEDIATE` 事务；失败必须回滚 |
...
| 删除角色 | 200，逻辑删除 |
| 删除仍由启用用户持有的角色 | 409 `ENTITY_IN_USE`，角色与用户分配行不变 |
| 仅停用/已删除用户持有 | 200，角色软删除，历史用户分配行不变 |
