...
| 步骤 | 操作 | 说明 |
|------|------|------|
| 1 | `BEGIN IMMEDIATE` 后查询未删除用户 | 404 若不存在 |
| 2 | 锁内查询活引用 | 生效且未删除项目的 `manager = username`，或未完成/未取消且未软删出库的 `operator = username` |
| 3 | 活引用存在 | 409 `ENTITY_IN_USE`，回滚且零业务写 |
| 4 | 无活引用 | UPDATE `is_deleted = 1` 后 COMMIT |
| 5 | 返回 200 | 历史行不级联、不改写 |
...
| 活持有阻断 | 生效项目负责人或在途出库经办属于现行分配，删除返回 409 `ENTITY_IN_USE` |
| 历史引用放行 | 停用/已删除项目、已完成/已取消/已软删出库和操作日志均为历史引用，不阻断删除 |
| 未知状态 fail-closed | 出库状态只有 `completed`/`cancelled` 放行；空值、未知值按活引用阻断 |
| 删除后影响 | 历史操作日志和业务单据仍保留，不级联删除或改写操作者 |
| 原子性 | 查询用户、锁内查引用、软删除写入处于同一个 `BEGIN IMMEDIATE` 事务；失败必须回滚 |
...
| admin 删除用户 | 200，逻辑删除 |
| 删除生效项目负责人 | 409 `ENTITY_IN_USE`，用户和项目行不变 |
| 删除在途出库经办用户 | 409 `ENTITY_IN_USE`；未知出库状态同样阻断 |
| 仅历史单据/日志引用 | 200，用户软删除，历史行逐字段不变 |
