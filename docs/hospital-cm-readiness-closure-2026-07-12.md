...
> 2026-07-16 名单方向更新：PM 已批准以管理员维护的版本化医院目录配置决定“谁应出现”（[Issue #182 权威 ASCII 决策记录](https://github.com/Mazikorn/Coreone-Procurement-Sales-and-Inventory-PSI-Management-System/issues/182#issuecomment-4987580797)）；状态为 `APPROVED_NOT_CONNECTED`。该决定不证明目录运行时、金额来源、真实 finality 或 readiness 已完成。
...
两类 profile 的依赖只允许单向传播：`cmValueProfile` 或院级 CM 分子变化会要求组合覆盖倍数重算；`portfolioDenominatorProfile` 变化不得反向改变院级 CM 值、周期 validation currentness 或连续三期资格。C1 既有 `profile_fingerprint/profile_recipe_version` 列自 `C1.cm-value-profile-recipe.v2` 起只承载 `cmValueProfile`；旧 `C1.profile-recipe.v1` 摘要无法安全拆分，必须返回 `PROFILE_RECIPE_UPGRADED` 并由后续受控写入链重验，不原地改写旧证据。

#186 / D2 B0 的账户名册表只保存 `UNCONFIRMED_SINGLE_SOURCE_SNAPSHOT_NOT_AUTHORITATIVE_UNION` 候选快照。它不会自动生成 C1 `scope`、不会改变 readiness，候选 `contentHash` 也不能直接冒充 C1 的 `roster_source_hash`。后续只允许由已实现的版本化医院目录配置按服务月枚举稳定 `partners.id` 集合，再经受控桥接流程发布 C1 范围快照；B0 可以作人工核对输入，不能自动升级为配置权威。

C1 的证据时间全部由服务器时钟生成，调用方不能回填时间；证据引用只接受无查询、无片段、无本地路径的脱敏不透明引用（例如 `sanitized://...`、`roster://...`、`manifest://...`），不得把患者名、真实文件路径或带令牌 URL 写入不可变证据表。

回填按固定优先级执行，避免人工挑状态：

1. 先从 #182 已批准但尚未接线的版本化医院目录配置枚举院月，不从病例、结算文件或其他已入系统事实反推全集。
...
账户全集必须来自 #182 已批准的版本化医院目录配置，不得从“已经进系统的数据”反推。stablePartnerId **必须等于现有 `partners.id`**；管理员另维护账户编码、展示名、别名、是否纳入 hospital-cm 及生效/失效月份。included=true 时 effectiveFromMonth 必填；若只知道当前名单，安全默认从配置启用月生效，更早月份保持 scope 缺失/不完整与 REVIEW_REQUIRED。legacy 或导入自动创建的 partner 默认不纳入。范围快照的**存储/哈希/失效机制仍只归 C1**（`hospital_cm_month_scope_snapshots`：`accounts` 钉死为 `partners.id`，非 complete 或缺失一律 fail-closed），D2 不建第二套范围模型。

目录桥接使用现有 C1 `roster_source_hash`，只绑定配方版本、serviceMonth 与稳定排序后的 `partners.id` 成员投影；C1 继续按活代码计算 `scope_hash = sha256({ serviceMonth, accounts, rosterSourceHash })`，禁止新建 `rosterScopeHash`。这是待桥接 PR 落实的 target contract：current C1 注释仍描述旧多源名册内容，且任一新 eventNumber 都会严格失效旧 run。桥接必须先比较 current complete scope 与成员 hash，完全相同则不调用保存；底层严格失效测试保留，另补桥接 no-op 测试。只改展示名不得产生新 scope 或撤销院级历史 CM；纳入/退出和生效区间变化只发布受影响月份。账户编码或别名改绑使用独立 `mappingEvidenceHash`，不进入 roster/scope hash，只重审真正受影响的来源和月份。合作形态、当月活动、收入/成本完整度属于金额与覆盖证据，不进入成员 hash，也不能把未知折成 0。D 后续仍需采集：

1. 医院目录配置版本、稳定身份、编码/别名、纳入状态及有效期；
2. 有效期化的合作形态：全流程、代送加做、纯代阅片、纯外送、远程会诊；
3. 无病例号收入的规范行、来源文件哈希、结算月和业务线；
4. 纯外送的外部实验室应付与物流可变成本；纯会诊的专家费/平台费或明确固定成本属性；
5. 覆盖状态 `FULL / PARTIAL / NONE`，其中 NONE/UNMEASURED 的金额和比率为 `null`。

只要配置全集或可测收入分母不完整，未测占比必须为 `null`；页面只显示已识别未测账户数、已知金额和缺失原因，不给百分比或“下限”。只有以下机器条件同时满足才可展示下限：目标月配置 scope 完整；财务金额来源完整；可测账户收入金额完整；所有金额未知账户都已明确属于未测集合；所有纳入金额非负且分母大于 0；金额属于同一期间、币种和业务口径；可测/未测账户集合互斥、无重复。此时下限为 `已知未测金额 ÷ (已知可测金额 + 已知未测金额)`；任一条件失败（包括退款、冲销或负收入）都返回 `null`，不得声称下限。贡献毛利与 CSV 数值留空，不渲染为 0，也不进入排序。
...
2. **已决定，不再待拍**：每月完整医院范围以管理员维护的版本化医院目录配置为权威全集；这是 `APPROVED_NOT_CONNECTED`，不等于运行时和真实数据已接入。
