# hospital-cm 月份窄 delta mockup（E0 APPROVED）

> **状态：E0 APPROVED — PM 已批准四项月份 delta 交互合同。** 这不是生产页面、运行时 readiness 证据、业务验收或解锁授权，也不授权启动 E1。
...
1. 未选月份时保持校准，不自动代选月份。
2. 就绪判断月份与完整体检月份使用 exact month，必须完全一致。
3. 切月时先清旧完整内容和旧证据，再检查新月份。
4. 每月 readiness 与固定池版本/认账状态同屏说明。

PM 已明确批准本 E0；本批准只冻结四项月份 delta 交互合同，不授权启动 E1 生产前端，也不替代 readiness、golden、业务验收或生产解锁证据。
