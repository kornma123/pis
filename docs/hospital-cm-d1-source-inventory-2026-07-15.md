...
>
> **现场 base**：origin/master@4cbb26f551c7dc8f002c609ae0104fe590e60e4b，2026-07-15 只读取证
>
...
- 没有把 #182 的 candidate 名册升级为权威名册。
- 没有实现或复制 #183 的 manifest、范围快照、close/reopen revision、validation run/check、`cmValueProfile` 或 `portfolioDenominatorProfile` 指纹。
...
| 现有关账表保存当前 close/reopen 字段 | reconcile_hospital_months schema | 当前状态不是不可变 revision 证据；D1 必须消费 #183 的单调事件 |
...
2. #183 单一底座保存并现算的范围、revision、validation、`cmValueProfile` 与 `portfolioDenominatorProfile` 证据；两类 profile 必须隔离消费；
...
D1 不新建第二套 manifest 表、范围表、关账 revision、validation engine、profile engine 或 ready 开关。若 #183 的字段仍不足，先回到 #183 冻结共享合同，再由 D1 消费；不得在 D1 私下补一个近似模型，也不得把 `cmValueProfile` 与 `portfolioDenominatorProfile` 合并成同一失效域。
...
1. #183 C1 单一底座已合入所选 base，manifest/scope/revision/validation、`cmValueProfile` 与 `portfolioDenominatorProfile` 的隔离合同和回滚边界已冻结；
2. #182 的权威来源方向已由 PM/财务 owner 批准，至少一个候选历史月有真实、脱敏、受控的完整名册最小切片；
3. 财务结算、LIS、成本/公式与 finality 的 owner、保管人和独立 reviewer 已具名；
4. 至少一个历史月的财务/LIS/cost source manifest、版本/hash、控制数和重取路径可复查；
...
- [x] 明确只消费 #183 单一底座，不复制 manifest、范围、revision、validation、profile 或 ready 状态机；`cmValueProfile` 与 `portfolioDenominatorProfile` 保持隔离。
...
|---|---|---|---|
| 是否批准 #182 推荐的“合同有效账户台账 + 财务结算月度名册受控并集”为权威全集 | PM + 财务数据 owner | 决定 D1 从哪里枚举院月 | 不接真实名册，不从病例反推，保持 REVIEW_REQUIRED/null |
