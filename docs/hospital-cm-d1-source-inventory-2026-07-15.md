...
> **状态**：D1-0 开工合同（文档增量）；医院全集政策为 `PM_APPROVED / APPROVED_NOT_CONNECTED`，所有财务、LIS、成本、finality 与真实历史月份仍为 CANDIDATE / UNVERIFIED
>
> **风险档**：R2（历史收入、成本、关账、医疗业务数据与经营判断）
>
> **现场 base**：origin/master@388c3cd92ab0f22fa39c2732a37dc3776c722528，2026-07-16 重新取证
>
> **业务边界**：本文只定义“以后怎样证明一个历史院月可信、怎样重算、什么变化必须撤销旧结论”。本文不接真实来源、不写回填数据、不修改 readiness/API/UI、不解锁 /full-health，也不证明任何历史月份已可信。

## 0. 给 PM 的一句话结论

现在可以先把历史数据的证据合同定清楚，但还不能把任何旧月份升级为“已验证”。D1 必须先从 #182 已批准但尚未接线的版本化医院目录配置枚举“这个月本来应该有哪些账户”，再绑定财务、LIS、关账和成本版本证据；缺任何关键金额或 finality 证据都保持空值，不能用 0、插值、旧 PR、文档说明或测试 fixture 补洞。

本增量完成后，团队得到的是一张可执行的来源盘点表、固定回填顺序、失效矩阵和后续开工闸。真实回填与展示仍分别等待 D1-1、D1-2。

2026-07-16 对 current master 的复核确认：PR #203 已把 `cmValueProfile` 与 `portfolioDenominatorProfile` 的 currentness/失效域拆开，固定池 owner/金额/认账变化不再使院级周期 validation run 失效。这项机制前置已经满足，D1 必须直接复用，不能私建第二套 profile。**它仍不授权 D1-1**：医院目录运行时与 C1 真实配置 scope、财务/LIS/成本/finality 真实来源、具名 owner/reviewer、#183 剩余 C2/C3/C4 以及适用时的 #163 阶段 2 仍未完成。
...
- #182 已批准“管理员维护的版本化医院目录配置”作为医院全集政策，但目录运行时、C1 配置 scope 和真实配置月份尚未完成；B0 candidate 没有因此升级。名单成员资格不再需要额外 roster RATIFIED，金额来源仍须各自具名 owner、受控 manifest/hash、守恒与真实脱敏切片。
- 没有在 D1 中实现或复制 #183 的 manifest、范围快照、close/reopen revision、validation run/check 或 profile 引擎。current master 的 C1 提供共享底座，PR #203 已提供双 profile 隔离；D1 只消费它们，#183 的真实 finality、受控 writer/check 集与 C2/C3/C4 仍是未完成门。
...
| #182 的医院全集政策已由 PM 更新批准 | [#182 权威 ASCII 决策评论](https://github.com/Mazikorn/Coreone-Procurement-Sales-and-Inventory-PSI-Management-System/issues/182#issuecomment-4987580797) 批准管理员维护的版本化医院目录配置 | 不再重复询问名单是否完整，也不附加 roster RATIFIED；状态仍是 APPROVED_NOT_CONNECTED，B0 不自动升级，财务/LIS/成本/finality 继续各走 manifest/hash/owner/守恒 |
...
| 院级 CM 与固定池 profile 已隔离 | [PR #203](https://github.com/Mazikorn/Coreone-Procurement-Sales-and-Inventory-PSI-Management-System/pull/203) 已合入 current master；固定池控制变化不再改变院级 profile currentness，并有负向隔离测试 | 机制前置已满足；D1 必须复用。该事实不替代目录运行时、真实 source/finality、C2/C3/C4 或 D1-1 独立开工闸 |
...
2. #183 单一底座保存并现算的范围、revision 与 validation 证据；账户 CM value profile 和固定池 denominator profile 使用 PR #203 已合入的独立 currentness/失效域；
...
## 3. 枚举顺序：必须先从 #182 医院目录配置开始

固定顺序如下，任何实现不得跳步：

1. 读取目标 serviceMonth 的 #182 版本化医院目录配置，并按 included 与 effectiveFrom/effectiveTo 投影稳定 accountId 集合；included=true 必须有明确 effectiveFromMonth，未知历史起点不得向更早月份倒灌。
2. 只有目录配置运行时已实现、该月 C1 scope 为 current/complete、成员投影 hash 当前有效，才按稳定 accountId 排序枚举账户；管理员保存配置就是名单政策动作，不再等待独立 roster RATIFIED。
...
- 配置功能尚未实现或尚未发布该月 C1 scope：D1-1 不启动，该月保持 REVIEW_REQUIRED/null；
- 配置历史可恢复但当前 revision/scope 缺失：登记缺失配置版本、责任人与重取路径，保持 REVIEW_REQUIRED/null；
- 已证明稳定身份或历史配置投影永久不可恢复：该月为 UNVERIFIABLE，数值为 null；
...
> `D2_ACCOUNT_ROSTER_CONFIG` 是已批准但尚未接线的名单控制面，不是金额证据；除该行外，本节财务、LIS、成本、finality 与真实历史来源均为 **CANDIDATE / UNVERIFIED**。表中的“需要的受控位置/版本/hash”是准入合同，不表示真实来源已经存在或已经取得。

| 候选 sourceId / 来源 | source owner | 期间与月轴 | 需要的受控位置、版本与 hash | 最小字段 | 脱敏等级 | 缺失时降级 | 重取或重审触发 | 当前状态 |
|---|---|---|---|---|---|---|---|---|
| D2_ACCOUNT_ROSTER_CONFIG：#182 版本化医院目录配置 | 有权管理员/partner 主数据 owner；配置动作需审计，不另设 roster RATIFIED | 目标 serviceMonth；included 与 effectiveFrom/effectiveTo 投影；included=true 必须有明确起始月 | 运行态配置 ID/revision、scope recipe version、现有 C1 rosterSourceHash/scopeHash；真实名称不入 Git | stablePartnerId=partners.id、accountCode、aliases、hospitalCmIncluded、effectiveFrom/To；display name 只供展示 | L1 | 未实现或目标月 scope 缺失则 D1-1 阻塞并保持 REVIEW_REQUIRED/null；稳定身份永久不可恢复才 UNVERIFIABLE | 纳入/退出、有效期、身份合并拆分；code/alias 改绑只重审受影响映射；展示名变化不失效数值 | PM_APPROVED / APPROVED_NOT_CONNECTED；B0 不自动升级，尚无运行时或真实配置月 |
...
上述清单是 D1 的**准入合同**，不是 current master 已具备真实来源、finality 或回填能力的声明。current master@388c3cd9 已有 C1 manifest/scope/close revision/validation 存储和读侧失效，PR #203 又把固定池从院级 `cmValueProfile` 的 currentness 中移出；只改固定池时院级 CM/`PERIOD_QUALITY_VERIFIED` 保持 current。

D1 不新建第二套 manifest 表、范围表、关账 revision、validation engine、profile engine 或 ready 开关。它直接消费 #183/PR #203 的共享双失效域合同，并在 D1 回归中继续证明只变固定池时账户 CM run 与历史质量资格保持 current；不得在 D1 私下补一个近似模型，也不得把 `cmValueProfile` 与 `portfolioDenominatorProfile` 合并成同一失效域。

目录桥接还必须适配 current C1 的严格事件语义：raw `saveMonthScopeSnapshot` 即使保存同内容也会因新 eventNumber 使旧 run 失效。桥接先比较 current complete accounts 与 rosterSourceHash；相同则 no-op，不调用 save。mappingEvidenceHash 不进入 C1 rosterSourceHash/scopeHash。桥接 PR 必须保留底层“新 event 严格失效”测试，并新增桥接 no-op、展示名不失效和映射局部失效测试；否则 D1-1 不得消费。
...
- serviceMonth；
- C1 当前 scopeHash（由同月排序后的 partners.id accounts 与 rosterSourceHash 唯一派生）；rosterSourceHash 只绑定配方、serviceMonth 与成员投影，不绑定 display name 或整个配置 revision；
- 实际用于来源归属的 mappingEvidenceHash（如有）；code/alias 改绑只进入受影响来源的运行输入；
...
1. 校验目标月配置投影与 C1 scope current/complete；不另查 roster RATIFIED；
...
- 只改 canonical display name 或无关展示字段不得改变 C1 rosterSourceHash/scopeHash、mappingEvidenceHash 或 `cmValueRunKey`。
...
|---|---|---|---|---|
| 只改医院 canonical display name 或其他无关展示字段 | 不失效任何 scope、映射或数值证据 | 原 `cmValueRunKey`、账户 CM 与历史状态保持 current，只刷新展示 | 审计化配置 revision | 把 display name 或整个 configRevision 编入 scope/runKey |
| hospitalCmIncluded、effectiveFrom/To 或稳定身份合并拆分变化 | 只影响成员投影发生变化的 serviceMonth；该月 scope、周期资格与组合汇总重评估，区间外月份不变 | 受影响月旧 scope/run 不可消费；null + REVIEW_REQUIRED；若历史身份投影永久不可恢复则 UNVERIFIABLE | 新配置投影、C1 scope hash、审计记录与范围回归 | 从病例表补回遗漏账户，或全历史无差别失效 |
| accountCode/alias 改绑 | 只重审实际使用该映射的来源与月份；成员集合不变时 C1 rosterSourceHash/scopeHash 不变 | 仅当来源行归属或金额输入变化时产生新 `cmValueRunKey`；无关历史结果保持 current | 新 mappingEvidenceHash、冲突/未命中账与受影响范围证明 | 因别名文字变化撤销所有月份 |
...
1. 所选 base 必须包含 #183 C1 单一底座与 PR #203 的 `cmValueProfile` / `portfolioDenominatorProfile` 隔离合同；current master@388c3cd9 **已满足这项机制前置**，但 #183 剩余 C2/C3/C4 与真实 finality 仍按实际依赖阻断业务通过；
2. #182 医院全集政策已由 PM 批准；目录配置运行时和上述 C1 bridge/no-op/mapping 隔离合同必须已实现并通过负向测试，再向 C1 发布至少一个真实配置月份的 current/complete scope。政策批准不替代运行时或数据就绪，也不再追加 roster RATIFIED；
...
- [x] 医院全集政策明确标为 PM_APPROVED / APPROVED_NOT_CONNECTED；所有金额、LIS、成本、finality 与真实历史来源仍标 CANDIDATE / UNVERIFIED，未知 owner 明确待具名。
- [x] 枚举顺序从 #182 版本化医院目录配置开始，不从病例或工作流反推全集。
...
- [x] 记录 PR #203 已在 current master 完成双 profile 隔离，并保留“固定池变化不得撤销院级历史 CM”的负向合同；未把机制合并误写成真实来源或 D1-1 已解锁。
...
- [ ] 至少一个脱敏真实历史失真院月可从 #182 配置 scope → mapping/source manifest/batch/hash → close revision/finality → `cmValueProfile` → 状态/账户 CM 值完整复算；组合覆盖倍数另经 `portfolioDenominatorProfile` 复算。
...
- [ ] 缺 source、配置 scope 不完整、finality 不足或无法证明原 `cmValueProfile` 的月份诚实返回 REVIEW_REQUIRED/UNVERIFIABLE 与 null，不显示 0、不插值；仅缺固定池时账户 CM 仍可返回，只有 `coverageMultiple` 为 null 并带原因。
- [ ] RESTATED_CURRENT_BASIS 与 PERIOD_QUALITY_VERIFIED 在 API、页面、趋势和 CSV 中不混义；重述不计入连续三期。
- [ ] #182 配置运行时未覆盖的月份不能进入真实回填验收；机制完成不能冒充业务完成。
...
已拍决定（不再重复询问）：PM 于 2026-07-16 明确完整医院名单已提供，并批准以管理员维护的版本化医院目录配置作为 hospital-cm 医院全集权威；见 [#182 权威 ASCII 决策记录](https://github.com/Mazikorn/Coreone-Procurement-Sales-and-Inventory-PSI-Management-System/issues/182#issuecomment-4987580797)。管理员保存配置即名单政策动作，不再等待外部名单确认或独立 roster RATIFIED。该决定不把 B0 升级为配置、不证明目录运行时或任何金额/finality 来源已完成，也不授权 D1-1。
...
- 院级 CM / 固定池双 profile 隔离：[PR #203](https://github.com/Mazikorn/Coreone-Procurement-Sales-and-Inventory-PSI-Management-System/pull/203)
...
- #182 已批准配置合同：[医院全集来源决定与配置合同](hospital-cm-account-roster-source-decision-2026-07-14.md)
...
第一步永远先读医院目录配置“这个月应该有哪些账户”，再逐家核对结算、LIS、关账和当时成本版本。证据全，才画实线；只能用今天的口径重算，就明确写“按当前口径重述”；还在补证或已经无法核实，就留空并断线。本文只是把这套规则写清楚，真实数据、代码和页面都还没有因此完成。

医院全集方向已经拍定为版本化配置，但配置运行时和真实配置月份还没有接上；财务、LIS、成本与 finality 的真实证据和具名责任人也没有到位。PR #203 已把“院级 CM 证据”和“组合固定池证据”的失效域拆开，所以这项机制不再重复阻塞。下一步仍不是回填，而是先实现目录控制面并发布 C1 scope，再让真实来源、C2/C3/C4、finality 与独立复核逐项过门。
