...
- `kind` 仅允许 `unverified|untested|unmeasured|unknown|assumption|dependency|risk`；`detail` 必须是非占位、可见的纯文本。anchor typed value 与完整 uncertainty typed value 的 UTF-8 上限分别为 512、2048 bytes；合同内只允许 parser 声明的字母/数字/普通空格和有限可见标点。完整支持的 entity 解码后或直接书写的裸 `&` 可作为 `R&D+`、`A&B` 这类普通可见文本的一部分；无分号的受支持 entity 名或至少两个字符的名称前缀 fail-closed，仍拒绝 Markdown/HTML 包装、反斜线、未解析 entity 与分隔符注入。
