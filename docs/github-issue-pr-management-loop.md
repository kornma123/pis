...
Agent 认领前先核对该块、GitHub assignee、开放 PR 和 worktree；认领成功后先更新该块，再发一条认领评论。没有 Issue body 写权限时只能提出认领，不得把自己视为 current owner；由有权角色回填后才生效。GitHub assignee 可以镜像 owner，但与正文冲突时先停下并校准，不能选择性取用。
