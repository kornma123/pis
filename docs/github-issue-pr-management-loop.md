...
两项反盲区回答在 PR body 与 Issue handoff 评论中使用同一严格机器合同：`risk-v1; anchor=<type>:<value>; uncertainty=<kind>:<detail>` 或 `no-finding-v1; checked=<type>:<value>; unchecked=<type>:<value>`；Issue 评论字段名为 `least-confidence` 与 `biggest-missing`。旧 free-form 没有兼容 fallback；ASCII mode/key/id 是 entity decode + NFKC 后的 canonical 形态，raw 与 canonical contract 都须 `<=4096` UTF-8 bytes，ref 正整数始终以 digit string 保真。canonical mode 后首个 `;` 固定分段；其余 `;` 仅在后继 optional space/tab + ASCII field-key + optional space/tab + `=` 时才是字段分隔，再逐 token 重检 entity，不能按 value 大小写猜测；NFKC 后新出现的 unresolved entity 也 fail-closed。placeholder 比较忽略连续句末标点及 `/ _ + - &` 终止填充，并拒绝 canonical `n(?:[./_+-])?a` 等价族；内部的 `C++`、`snake_case`、`R&D+`、`A&B`、`rock&roll` 与路径保持不变。完整支持的 entity 可解码，裸 `&` 可作可见文本；真正的未知完整 entity 保留自身 `;`，非末字段再用独立 grammar `;` 分隔并 fail-closed。entity 名边界按 ASCII 名 token 推导，无分号的受支持 entity 名/前缀 fail-closed。完整 lexical 规则以共用契约 §6.1 为准。checker 只证明 wire shape / lexical anchor，不证明声明为真或检查确已执行；reviewer / PM 必须继续审查内容真实性。最终用户交付仍用产品大白话。

跨会话或跨模型交接时，下一位执行者先读取主 Issue、PR body、最新 checks 和当前 Git 状态，再开始工作；不继承上一会话口头声称的“已完成”。同一文件同一时间只有一个实现 owner，复核模型不在被审文件上代写。

## 5. 线下检查做什么

PR body 在发布前保存为本地 UTF-8 文件并运行：

```sh
node scripts/issue-handoff/check-pr-body.cjs --body-file <path>
node scripts/issue-handoff/check-pr-body.selftest.cjs
...
上述本地 PASS 只证明交接格式和治理负控成立，不替代目标 SHA 的测试、独立复核、PM 发布/合并批准或 GitHub 当时的 mergeability。文件或 SHA 改变后必须重跑对应本地证据。

## 6. 合并与关闭规则

- 完整交付：PR body 使用 `Closes #N`；常规 checks、线下固定 SHA 异构复核和 PM 合并批准齐全后人工合并；GitHub 关闭主 Issue。
...
- 每次 PR 合并前：在线下执行一次 follow-up sweep，确认没有未入 Issue 的后续工作；需要新 Issue 时先交 PM 确认，再由唯一写 owner 低频创建。
...
实现状态、阻塞和复核原文默认在线下记录并由 PM 粘贴交接，不承担第二份 backlog。每份文档至少包含：**状态或结论、目标固定 SHA、独立性、业务影响、证据、风险或未验证项、下一 owner 与触发条件**；不能只写“已检查”。
...
> 本文档默认不发布到 GitHub，也不改变正式 review state；只有 PM 明确批准具体仓库、ref、fixed SHA、文件/文本范围和动作时，唯一写 owner 才发布摘要、APPROVE / REQUEST CHANGES / MERGE。
```
...
以后每项工作仍有一个低频“总单号”（Issue），每次正式交付仍有 PR；但 PR 正文检查、K3/Codex 复核原文、findings 处置和后续项起草都先在线下完成。你点头后才由一个 owner 把必要的分支、PR、Issue 或最终决定摘要发到 GitHub，不再让机器人自动写 status/review/comment，也不把模型聊天和过程进度搬上去。
