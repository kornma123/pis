...
> PR / Issue 机器入口只接受共用契约 §6.1 的 `risk-v1` / `no-finding-v1` typed wire grammar，不接受旧 free-form，也不要带 Markdown/HTML 包装。ASCII mode/key/id 是 entity decode + NFKC 后的 canonical 形态；raw 与 canonical contract 都须 `<=4096` UTF-8 bytes，ref 编号按 digit string 比较。canonical mode 后首个 `;` 固定分段；其余 `;` 仅在后继 optional space/tab + ASCII field-key + optional space/tab + `=` 时才是字段分隔，再逐 token 重检 entity，不能按 value 大小写猜测；NFKC 后新出现的 unresolved entity 也 fail-closed。placeholder 比较会忽略连续句末标点及 `/ _ + - &` 终止填充，并拒绝 canonical `n(?:[./_+-])?a` 等价族；内部的 `C++`、`snake_case`、`R&D+`、`A&B`、`rock&roll` 与路径保持不变。完整支持的 entity 可解码，裸 `&` 可作可见文本；真正的未知完整 entity 保留自身 `;`，非末字段再用独立 grammar `;` 分隔并 fail-closed。entity 名边界按 ASCII 名 token 推导，无分号的受支持 entity 名/前缀 fail-closed。checker 只校验 wire shape 与 lexical anchor；名称真实性、检查是否实际执行和回答是否诚实仍须人审。最终直接面向用户的交付结尾继续使用产品大白话。
