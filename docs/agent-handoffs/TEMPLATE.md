...
> PR / Issue 机器入口只接受共用契约 §6.1 的 `risk-v1` / `no-finding-v1` typed wire grammar，不接受旧 free-form，也不要带 Markdown/HTML 包装。ASCII mode/key/id 是 entity decode + NFKC 后的 canonical 形态；raw 与 canonical contract 都须 `<=4096` UTF-8 bytes，ref 编号按 digit string 比较。placeholder 比较会忽略连续句末标点及 `/ _ + - &` 终止填充，并拒绝 canonical `n(?:[./_+-])?a` 等价族；内部的 `C++`、`snake_case`、`R&D+`、`A&B` 与路径保持不变。完整支持的 entity 可解码，裸 `&` 可作可见文本；无分号的受支持 entity 名/前缀 fail-closed。checker 只校验 wire shape 与 lexical anchor；名称真实性、检查是否实际执行和回答是否诚实仍须人审。最终直接面向用户的交付结尾继续使用产品大白话。
