...
> **状态**:v1.6 **文档定稿 · 决策台账零待 PM 选择**(2026-07-24)。PM 已先明确说「定稿」并在 2026-07-15 批准 §10 剩余推荐、指定 O-2 四个角色统一称「管理员」;2026-07-16 又将 O-1 更新为管理员维护的版本化医院目录配置;2026-07-24 最终确认进销存独立、医院端只做 BOM 材料成本与贡献毛利，ABC 工时/工资/设备/场地/间接费用产品面退出当前范围。原仓已停止作为实时协作入口,历史链接只保留 provenance;当前执行状态、owner、PR 和 checks 统一从 [`kornma123/pis` Issues](https://github.com/kornma123/pis/issues) 与对应 PR 现场读取。[2026-07-20 本地 Issue 指导](prd/COREONE-本地Issue指导-2026-07-20.md) 已降为历史规划快照，不得作为当前派工看板。这里的「文档定稿/决策清零」只确认本文可作为后续需求输入与范围导航;不等于实施完成,不升级 B/C、RATIFIED 或 Locked Golden,不代表部署/上线、真实业务验收或对外承诺。
> **修订说明**:v1.0(2026-07-14)首版;v1.1(2026-07-15)逐项处理历史 PR #190 的既有复核与动态事实;v1.2 记录文档定稿决定及其权限边界;v1.3 将 §10 全部剩余推荐转为 PM 已决策项、记录 O-2「管理员」与独立性边界,并吸收 E0 mockup 已合入 master 的动态事实;v1.4 同步 O-1 版本化医院目录决定;v1.5 在新仓恢复历史后同步 live Issue 入口和治理索引;v1.6 以 [ABC 域退役与兼容边界](prd/COREONE-ABC域退役与兼容边界-2026-07-24.md) 推翻旧“ABC 页面保留/共存”决定，并冻结 BOM 不反推库存的不变量。2026-07-24 的 Git 固定对象和 GitHub 状态只属于该次取证历史；实时状态一律现场查 GitHub/Git，不在本稳定 PRD 中复制移动分支 SHA。
...
| finance 财务 | 对账审批、材料成本/贡献毛利只读分析 | 不使用 `abc_config`/`labor_times` 产品能力；历史 capability 仅兼容，退役前不得据此恢复入口 |
| lab_director 主任 | 材料成本/贡献毛利只读参考 | 不使用标准工时、设备折旧、工资、场地或 ABC 配置产品能力 |
...
- **BOM 不反推库存(2026-07-24 PM FINAL)**:BOM 只用于材料配方、成本测算与推荐。读取、写入、核准、版本化或计算 BOM 均不得预占、扣减或增加库存；只有真实进销存业务单据可以改变批次与库存事实。若未来由 BOM 辅助生成出库草稿，草稿生成本身仍不得落库存。
...
### 4.8 报表与 ABC 域(🛑 2026-07-24 PM FINAL：ABC 产品面退出当前范围)

旧“ABC 配置页保留、报表并存”的 2026-07-03 决定已被后续 PM 决定推翻。当前产品不继续开发标准工时、人员工资、设备折旧、场地、间接费用分摊、activity center/cost pool/cost driver 或 ABC 全成本报表；医院端只展示基于 BOM 的材料标准成本与贡献毛利，固定成本池只作为整盘月度汇总数参与覆盖倍数，不分摊到医院/病例/BOM。仓库现存 ABC 页面、路由、表与 helper 只按历史实现/兼容债务处置，不构成功能授权；先退役产品面，再解耦主线复用，最后在正式 migration 合同下处理数据面，禁止按 `abc_*` 名称批量删除。完整边界与顺序见 [ABC 域退役与兼容边界](prd/COREONE-ABC域退役与兼容边界-2026-07-24.md)。
...
| ABC 产品面退役 | 导航/Dashboard/可达路由不再暴露 ABC、工时、工资、设备折旧、场地和间接费用；主线兼容 helper 不误删 | 路由+DOM+调用图,yes/no |
| BOM/库存边界 | BOM 读写、核准、版本化和成本计算前后，批次及库存事实零变化；真实库存单据仍是唯一数量写入口 | API+数据库事务,yes/no |
...
**既有决定,直接承接勿重议**:B-5 双轴与三硬门;grade C 仅限内部探索且禁止对外/最终盈利判断/真实解锁证据;两层框架(收官页 §B+ADR-008);#163 身份键+分摊 Q2'=A;#156 五拍(2026-07-14);P-1 最低必要字段+导入即去标识化且无需患者明文回查;P-2 旧拆分结果存在重大高估风险但未对外使用、无需外部更正;#139 A 全批次+未指定批次 FEFO 跨批拆分+指定批次不足 422;项目仍处开发阶段、代码合入不等于生产上线;8 阈值保守默认;组织处理固定费政策已拍但具体值不在本 PUBLIC PRD 重复;EBER/特染归 IN;B-3 删幽灵入口;M-2 质疑档位用 R0–R3;M-6 构建纪律 C1/C2 block;退库/调拨语义;depletion 删除;O-9 本 PRD 完成文档定稿,只确认文档版本,不升级 B/C 权威、不授权对外输出或冒充业务实施完成。**2026-07-24 后续决定**:进销存为独立库存事实体系；医院端只以 BOM 推导材料标准成本和贡献毛利，BOM 不反推库存；标准工时、工资、设备折旧、场地、间接费用和 ABC 全成本产品面退出当前范围；旧 ABC 19 页保留决定不再适用，历史 capability 仅兼容、不得作为入口恢复依据。
