...
> **状态**:v1.2 **文档定稿**(2026-07-15)。PM 已在 v1.1 固定头 `72245c29` 的检查与复核完成后明确说「定稿」,并由 [PR #190 普通评论代录决定](https://github.com/Mazikorn/Coreone-Procurement-Sales-and-Inventory-PSI-Management-System/pull/190#issuecomment-4978904201)。这里的「文档定稿」只确认本文可作为后续需求输入与范围导航;不等于「口径定稿」,不升级 B/C、RATIFIED 或 Locked Golden,不代表部署/上线、真实业务验收或对外承诺,也不构成 PR 合并授权。
> **修订说明**:v1.0(2026-07-14)首版;v1.1(2026-07-15)逐项处理 PR #190 的[既有复核 P1/P2](https://github.com/Mazikorn/Coreone-Procurement-Sales-and-Inventory-PSI-Management-System/pull/190#issuecomment-4976504018)与动态事实,并落实[PM 拍板记录](https://github.com/Mazikorn/Coreone-Procurement-Sales-and-Inventory-PSI-Management-System/pull/190#issuecomment-4977403367);v1.2(2026-07-15)仅记录 PM 的文档定稿决定及其权限边界。首轮 4 镜头只留下汇总、逐项记录未进入仓库或 GitHub,故本版不再把「17/17」当可独立复核证据,边界与本轮可访问处置见 §13。本次取证基线 = 云端 `origin/master@f8a1a2cf` + GitHub 现场事实(2026-07-15);SHA 只是取证时点,实时状态一律现场查 GitHub/Git。
...
2. **作用 = 指导后续开发**:后续任何开工,先对齐本文的「现状/差距/口径/路线图/待拍决策」,再进对应域的权威文档与质量 Loop;本文回答「做什么、先做什么、什么不做、数字信到什么程度」,不逐条重写 API 细节(那是 FRS/域 spec 的活)。**PM 于 2026-07-15 随文档定稿确认。**
...
| B 固定成本池认账 | 不可变值版本+RATIFIED,值变即失效 | ✅ 控制面已实现并合入 master(PR#172);📋 真实金额+签字人(2026-08-31) |
...
| D2 账户全集与无病例号收入 [#182](https://github.com/Mazikorn/Coreone-Procurement-Sales-and-Inventory-PSI-Management-System/issues/182) | 版本化月度名册(权威源=合同有效账户台账∪财务月度结算名册,**O-1 已拍**)+无病例号收入/成本台账+UNMEASURED 返 null 披露覆盖 | ✅ B0 候选快照已由 PR#186 合入 master(无端点无消费者);📋 具名 owner、受控真实来源、守恒与消费者接线 |
...
### 4.10 平台与治理(✅ 骨架已实现并合入 master;🧊 数个决策票)
...
- **安全**:仓库代码止血已实现并合入 master(PR#119:缺 JWT 配置拒启动/禁弱密钥/生产模式不种默认账号),不冒充外部环境已处理;首次外部试用/正式上线安全门 runbook 就绪(目标环境 JWT 轮换/账号改密仍 NOT_TRIGGERED)。遗留决策票 #128(登录防暴力)、#149(AI 审查门密钥更名/孤儿配置)、#150(secret-scan 升 required+git 历史清史)。
...
> ⚠️ 表中月份与医院为示例位,实际验收月以届时已关账真实月为准(细节级假设,§12④)。本表是**基线定义**:注明「守住/已修」的行仅表示仓库实现/验证已达,不表示已部署;其余行在对应能力交付后才生效(现状分界见 §4)。**复算边界**:仓内代码与脱敏 fixture 可由 fresh clone 运行;16 院 Candidate 锚与 K-8 原始核价未交付同 hash 原件、受控逐锚明细/成员清单和复算包,当前仅持有同一 hash 原件者可复算。公开第三方只能核对本文披露的来源、等级与边界,不能独立重算这些值;#181 完成受控归档且工具/派生包可取得前,本文不作第三方可复算承诺。
...
**外部依赖(不受工程控制,是主线最大风险)**:①O-1 已确认两类名册存在、可导出且有统一稳定账户编号,但具名 owner、受控 source manifest、版本/hash、服务月映射与账户数/金额守恒仍未交付;②固定成本池真实金额与签字人;③CK广/CYCD-1 发票;④固定池签字人、数据保管人、首周期独立 reviewer、纯代送业务 owner、QA/E2E owner 与隐私合规 owner 仍待具名(未具名=机器红灯)。
...
**已拍板,直接执行勿重议**:B-5 双轴与三硬门;grade C 仅限内部探索且禁止对外/最终盈利判断/真实解锁证据;两层框架(收官页 §B+ADR-008);#163 身份键+分摊 Q2'=A;#156 五拍(2026-07-14);O-1 合同有效账户台账∪财务月度结算名册;P-1 最低必要字段+导入即去标识化且无需患者明文回查;P-2 旧拆分结果存在重大高估风险但未对外使用、无需外部更正;#139 A 全批次+未指定批次 FEFO 跨批拆分+指定批次不足 422;#135 finance 无 `abc_config/labor_times`、`cost_analysis:R`,lab_director 两项只读,角色并集不变,两个不同 admin 做提交/复核,前端写按钮跟随 capability;项目仍处开发阶段、代码合入不等于生产上线;8 阈值保守默认;组织处理固定费政策已拍但具体值不在本 PUBLIC PRD 重复;EBER/特染归 IN;B-3 删幽灵入口;M-2 质疑档位用 R0–R3;M-6 构建纪律 C1/C2 block;退库/调拨语义;ABC 19 页处置;depletion 删除;O-9 本 PRD 于 2026-07-15 完成文档定稿,只确认文档版本,不升级 B/C 权威、不授权对外输出、业务实施或 PR 合并。

**随文档定稿保留到后续专项决策/实现阶段**(编号沿用来源):PM 的「定稿」确认这些开放项不阻断本文成为上游输入,不等于接受下列推荐、解除红灯或授权开工;每项生效前仍须在对应 Issue 由具名 owner 补齐证据并单独拍板。
...
4. 决策清算:O-2(具名 owner)、O-3(mockup 四问)——不拍则 B/C/D2/E 仍被卡;O-1 已拍,转 #182 做真实脱敏来源与守恒。
...
10. 既有票消化:#129/#130/#135 精确权限矩阵实施/#150/#160;#139 决策已完成,其工程范围由 Now 的 #140 承接。
...
**方向级(定义见契约 §3;全部已转 §10 或 §0,零带标前进)**:①本文范围解读(§0 三问,PM 已于 2026-07-15 随文档定稿确认);②最近三个可回溯合格周期存在(O-5 兜底);③O-1 已确认来源方向与稳定账户编号,但具名 owner、真实脱敏 manifest、版本/hash、服务月映射与守恒未齐,缺一仍阻断 RATIFIED/readiness。
...
**2026-07-15 可访问修订输入**:PR #190 的[普通 Codex 复核](https://github.com/Mazikorn/Coreone-Procurement-Sales-and-Inventory-PSI-Management-System/pull/190#issuecomment-4976504018)(不改变 formal review state)与[PM 拍板记录](https://github.com/Mazikorn/Coreone-Procurement-Sales-and-Inventory-PSI-Management-System/pull/190#issuecomment-4977403367)。本轮逐项处置如下;「写入文档」只证明已实现,是否关闭 finding 必须由新 head 的独立复核判定,本文不自批通过。
...
本 PRD 已于 2026-07-15 完成**文档定稿**:PM 在 v1.1 固定头 `72245c29` 的检查与复核完成后明确说「定稿」,v1.2 只把该决定及其边界写入本文。文档定稿不关闭 §10 仍列出的业务开放项,不升级 B/C、RATIFIED 或 Locked Golden,不代表部署/上线、真实业务验收或对外承诺,也不构成 PR 合并授权。v1.2 新头仍须通过 CI 与固定 SHA 独立复核;这些结果只证明本次提交完整性,不能反向扩大 PM 的决定范围。
...
这份「整个项目的总需求书」现在已经**文档定稿**,可以作为后续需求讨论、薄 PRD 与开发排期的上游输入。它把系统分成十个部分,每个部分都写清楚三件事:**仓库真实做到了哪**(例如出入库防重复记账、医院贡献毛利 fail-closed 门已实现并合入 master,但项目尚未部署或承载真实日常业务)、**还差什么**(例如固定成本池真实金额、具名签字人、受控脱敏名册与真实周期)、**数字信到什么程度**(真数据核过/条件性推算/grade C 内部探索分开写,grade C 禁止对医院或第三方输出)。#187 的 C1 底座与 #189 的关账竞态修复已经合入;O-1、P-1/P-2、#139、#135 与 §0 三条方向已按 PM 决定写回。仍需逐项处理 §10 的开放决策、具名 owner 与真实数据硬门,这些不会因文档定稿自动变绿。本次只把总图定下来,不自动拆业务实现任务,也没有合并 PR #190;新头仍须通过 CI 和固定 SHA 独立复核,后续合并仍需另行满足合并门并取得 PM 明确授权。
