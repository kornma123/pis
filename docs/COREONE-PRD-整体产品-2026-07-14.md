...
> **状态**:v1.1 草案(2026-07-15)· **待 PM 定稿拍板**(按质量 Loop 契约,PM 未明确说「定稿」前本文保持草案且不得声称定稿;§5 台账仍含 B/C 级决策数字,故「口径定稿」同样不可标)。
> **修订说明**:v1.0(2026-07-14)首版;v1.1(2026-07-15)逐项处理 PR #190 的[既有复核 P1/P2](https://github.com/Mazikorn/Coreone-Procurement-Sales-and-Inventory-PSI-Management-System/pull/190#issuecomment-4976504018)与动态事实,并落实[PM 拍板记录](https://github.com/Mazikorn/Coreone-Procurement-Sales-and-Inventory-PSI-Management-System/pull/190#issuecomment-4977403367)。首轮 4 镜头只留下汇总、逐项记录未进入仓库或 GitHub,故本版不再把「17/17」当可独立复核证据,边界与本轮可访问处置见 §13。本轮取证基线 = 云端 `origin/master@a5cbca38` + GitHub 现场事实(2026-07-15);SHA 只是取证时点,实时状态一律现场查 GitHub/Git。
...
**口径定稿状态(诚实声明)**:K-9~K-14 仍含支撑决策的 B/C 级口径或内部结果指针 → 本文不标「口径定稿」。PM 已于 2026-07-15 接受 grade C 的**内部探索边界**,这不把 C 升成 A、不授权对外输出、不构成最终盈利判断或真实解锁证据;校准按契约 §5 入队,但本 PRD 未定稿前不拆新的业务实现任务。
...
**方向级(定义见契约 §3;全部已转 §10 或 §0,零带标前进)**:①本文范围解读(§0 三问,等 PM 确认);②最近三个可回溯合格周期存在(O-5 兜底);③O-1 已确认来源方向与稳定账户编号,但具名 owner、真实脱敏 manifest、版本/hash、服务月映射与守恒未齐,缺一仍阻断 RATIFIED/readiness。
...
本 PRD 仍是草案。PM 未明确说「定稿」、台账仍有未拍残留、阶段自检与最近一整轮 PM 反馈未全绿前,不得把本节或 CI 全绿写成定稿/合并授权。
...
这份文档就是你要的「整个项目的总需求书」。它把系统分成十个部分,每个部分都写清楚三件事:**仓库真实做到了哪**(例如出入库防重复记账、医院贡献毛利 fail-closed 门已实现并合入 master,但项目尚未部署或承载真实日常业务)、**还差什么**(例如固定成本池真实金额、具名签字人、受控脱敏名册与真实周期)、**数字信到什么程度**(真数据核过/条件性推算/grade C 内部探索分开写,grade C 禁止对医院或第三方输出)。#187 的 C1 底座与 #189 的关账竞态修复已经合入,不再列成在途;O-1、P-1/P-2、#139、#135 已按 PM 决定写回,不再重复询问。接下来仍需:①确认第 0 节三条方向解读;②只处理第 10 节剩余待拍项和具名 owner;③让新 head 通过 CI 与固定 SHA 独立复核。只有质量 Loop 通用定稿退出四条与 PRD 特有追加项全部全绿、且 PM 明确说「定稿」后,本文才成为后续开发输入;在此之前不拆新的业务实现任务、不合并。
