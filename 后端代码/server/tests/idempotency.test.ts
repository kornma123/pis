...
describe('入库/出库提交幂等键（防重复入账）', () => {
  ...
  it('IDEM-OUTBOUND-BOM: 同一幂等键两次 BOM 出库，库存与 ABC 成本只计一次', async () => {
    const suffix = `out-bom-${Date.now()}`
    const fixture = seedBaseFixture(db, suffix)
    const batchNo = `B-IDEM-${suffix}`
    ...
    const bomRes = await request(app)
      ...
      .set('Authorization', `Bearer ${token}`)
      .send({
        code: `BOM-IDEM-${suffix}`,
        name: '幂等BOM',
        version: 'v1',
        type: 'ihc',
        materials: [{ materialId: fixture.materialId, usagePerSample: 1, unit: '瓶', price: 10 }],
      })
    ...
    const projectRes = await request(app)
      .post('/api/v1/projects')
      .set('Authorization', `Bearer ${token}`)
      .send({ code: `P-IDEM-${suffix}`, name: '幂等BOM项目', type: 'ihc', bomId, status: 'active' })
    expect(projectRes.status).toBe(201)
    const projectId = projectRes.body.data.id

    const key = `idem-out-bom-${suffix}`
    const payload = { projectId, bomId, sampleCount: 10 }
    ...
    const outboundCount = (db.prepare('SELECT COUNT(*) as c FROM outbound_records WHERE id = ?')
      .get(first.body.data.id) as any).c
    const abcCount = (db.prepare('SELECT COUNT(*) as c FROM outbound_abc_details WHERE outbound_id = ?')
      .get(first.body.data.id) as any).c
    const inventory = db.prepare('SELECT stock FROM inventory WHERE material_id = ?').get(fixture.materialId) as any

    expect(outboundCount).toBe(1)
    expect(abcCount).toBe(1) // ABC 成本只写一次
    expect(inventory.stock).toBe(40) // 50 - 10，只扣一次
  })
