...
describe('charge-catalog：seed 逐条价格 = 真实目录价', () => {
  ...
  it('分子病理处理 012100000060000：¥100/次 × 1 = 100 / × 2 = 200（flat）', () => {
    expect(amt('012100000060000', 1)).toBe(100)
    expect(amt('012100000060000', 2)).toBe(200)
  })
  it('冷冻处理 012100000080000：5块94 / 7块 94+18×2=130 / 20块 94+18×15=364（无封顶，待核）', () => {
    expect(amt('012100000080000', 5)).toBe(94)
    expect(amt('012100000080000', 7)).toBe(130)
    expect(amt('012100000080000', 20)).toBe(364)
  })
  it('多重染色 012100000120001：1片410 / 4片 410×3+420=1650 / 13片 410×3+420×9+210=5220（stepped）', () => {
    expect(amt('012100000120001', 1)).toBe(410)
    expect(amt('012100000120001', 4)).toBe(1650)
    expect(amt('012100000120001', 13)).toBe(5220)
  })
