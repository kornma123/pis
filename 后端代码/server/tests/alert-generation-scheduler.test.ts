...
describe('LOC-031A transactional alert generation', () => {
  ...
  it('detaches the singleton when both generation and rollback fail', () => {
    const poisoned = db
    const prototype = Object.getPrototypeOf(db) as {
      exec(sql: string): void
      prepare(sql: string): unknown
    }
    const originalExec = prototype.exec
    const originalPrepare = prototype.prepare
    const execSpy = vi.spyOn(prototype, 'exec').mockImplementation(function (sql: string) {
      if (sql === 'ROLLBACK') throw new Error('rollback fault')
      return originalExec.call(this, sql)
    })
    const prepareSpy = vi.spyOn(prototype, 'prepare').mockImplementation(function (sql: string) {
      if (sql.includes('INSERT INTO alerts') && !sql.includes('current_stock')) {
        return { run() { throw new Error('generation fault') } }
      }
      return originalPrepare.call(this, sql)
    })

    expect(() => generateAlerts({ now: FIXED_NOW })).toThrow('Alert generation rollback failed')
    execSpy.mockRestore()
    prepareSpy.mockRestore()

    db = getDatabase()
    expect(Object.is(db, poisoned)).toBe(false)
    initializeDatabase()
  })
...
describe('LOC-031A real app startup boundary', () => {
  ...
  it('does not start the scheduler when disposed before the server begins listening', async () => {
    appModule ??= await import('../src/app.js')
    const runtime = appModule.startServer({
      host: '127.0.0.1',
      port: 0,
  ...
  it('allows only one active server runtime per process and releases the slot after dispose', async () => {
    appModule ??= await import('../src/app.js')
    const first = appModule.startServer({
      host: '127.0.0.1',
      port: 0,
      schedulerEnv: { ALERT_SCHEDULER_ENABLED: 'false' },
    })
    ...
    expect(() => appModule?.startServer({
      ...
      port: 0,
      schedulerEnv: { ALERT_SCHEDULER_ENABLED: 'false' },
    })).toThrow('COREONE server runtime is already active')

    await first.dispose()
    const replacement = appModule.startServer({
      host: '127.0.0.1',
      port: 0,
      schedulerEnv: { ALERT_SCHEDULER_ENABLED: 'false' },
    })
