...
describe('DELETE /api/v1/projects/:id — 项目活跃引用拦截', () => {
  it('项目已登记入统一目录（code_mappings project_code 别名）→ 409 ENTITY_IN_USE，零业务写 + 拒绝审计不含请求体', async () => {
    const id = 'PRJ-DG-CAT-LIVE'
    seedProject(id)
    ...
    expect(row('code_mappings', 'CM-DG-LIVE')).toEqual(before.mapping)
    expectSanitizedDenialAudit('projects', id)
  })

  it('项目有 LIS 病例引用 → 409 ENTITY_IN_USE', async () => {
    const id = 'PRJ-DG-CASE-LIVE'
    seedProject(id)
    seedLisCase('LC-DG-LIVE', id)
    const before = { project: row('projects', id), lisCase: row('lis_cases', 'LC-DG-LIVE') }
    ...
    expect(res.body).toMatchObject({ success: false, error: { code: 'ENTITY_IN_USE' } })
    expect(row('projects', id)).toEqual(before.project)
    expect(row('lis_cases', 'LC-DG-LIVE')).toEqual(before.lisCase)
    expectSanitizedDenialAudit('projects', id)
  })
  ...
  it('committed-race：锁前窗口被第二连接提交 LIS 病例 → 锁内重读拦截，零部分写', async () => {
    const id = 'PRJ-DG-CRACE'
    seedProject(id)
    const race = installCommittedRace(() => {
      rival.prepare(`INSERT INTO lis_cases (id, case_no, project_id, status) VALUES ('LC-DG-RACE', 'CASE-DG-RACE', ?, 'normal')`).run(id)
    })
    ...
    expect(res.body).toMatchObject({ success: false, error: { code: 'ENTITY_IN_USE' } })
    expect(row('projects', id)).toMatchObject({ id, is_deleted: 0 })
    expect(row('lis_cases', 'LC-DG-RACE')).toMatchObject({ id: 'LC-DG-RACE' })
  })
