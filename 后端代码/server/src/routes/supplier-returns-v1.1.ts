...
function loadAllocationRefundCap(db: any, ownerId: string): number {
  const rows = db.prepare(`
    SELECT a.quantity, COALESCE(b.inbound_price, m.price, 0) AS inbound_price
...
router.post('/', requireWriteAccess, (req, res) => {
  try {
    ...
    if ((batchId !== undefined && batchId !== null && batchId !== '') || inboundRecordId) {
      error(res, 'Batch or inbound selection cannot override automatic FEFO', 'FEFO_OVERRIDE_FORBIDDEN', 400); return
    }
    const db = getDatabase()
    ...
    try {
      ...
      `).run(
        ...
        null, reason, refund, trackingNo || null, operator || 'system', remark || null,
      )
