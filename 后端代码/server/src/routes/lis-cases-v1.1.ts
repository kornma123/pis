 ...
 *  - 导入（/import、/import-markers）= 口径与工作量数据源输入 → requireAnyRole('admin','finance') + reconciliation W。
 *  - 预览（/preview）只干跑、不落库 → requireAnyRole('admin','finance')。
...
router.post('/import', authenticateToken, requireImport, requireWrite, (req, res) => {
  try {
 ...
 */
router.post('/import-markers', authenticateToken, requireImport, requireWrite, (req, res) => {
  try {
    const db = getDatabase()
