...
router.get('/', (req, res) => {
  try {
    ...
    const count = (db.prepare('SELECT COUNT(*) as total FROM return_records WHERE is_deleted = 0').get() as any)?.total || 0
    const offset = (Number(page) - 1) * Number(pageSize)
    const list = db.prepare('SELECT * FROM return_records WHERE is_deleted = 0 ORDER BY created_at DESC LIMIT ? OFFSET ?').all(Number(pageSize), offset) as any[]
